...
class PdfStatementService {
  ...
  Future<Uint8List> buildStatement(MonthlyBill bill) async {
    ...
    doc.addPage(
      pw.MultiPage(
        ...
        build: (context) => [
          _header(bill, appIcon),
          pw.SizedBox(height: 14),
          _serviceInfoCard(bill),
          pw.SizedBox(height: 12),
          _financialSummary(bill),
          ...
          _statistics(bill),
          pw.SizedBox(height: 16),
          _sectionTitle('Calculation Breakdown'),
          pw.SizedBox(height: 8),
          _calculationBreakdown(bill),
          if (bill.settlement != null) ...[
            pw.SizedBox(height: 16),
            _sectionTitle('Payment Summary'),
            pw.SizedBox(height: 8),
            _paymentSummary(bill),
          ],
          if (bill.advances.isNotEmpty) ...[
            pw.SizedBox(height: 16),
            _sectionTitle('Advance Payments'),
            pw.SizedBox(height: 8),
            _advancePayments(bill.advances),
          ],
          if (bill.payments.isNotEmpty) ...[
            pw.SizedBox(height: 16),
            _sectionTitle('Payments'),
            pw.SizedBox(height: 8),
            _paymentHistory(bill.payments),
          ],
          if (_notes(bill).isNotEmpty) ...[
            pw.SizedBox(height: 16),
  ...
  List<_CalculationLine> _calculationLines(MonthlyBill bill) {
    ...
    return switch (service.templateType) {
      ...
      ServiceTemplateType.custom => _quantityCalculationLines(bill),
    };
  }
